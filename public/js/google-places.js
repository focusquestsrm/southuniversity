(function () {
  ...
  const stateKey = '__southGooglePlacesState';
  ...
  function closeSuggestions() {
    if (document.body && document.body.classList) document.body.classList.add('south-places-selection-complete');
  ...
  function handlePlaceChanged(autocomplete) {
    try {
      ...
      if (components.length === 0) {
        ...
        dispatch('south:address-incomplete', { missing: ['lead_address_address_visible'] });
      ...
      closeSuggestions();
      dispatch('south:address-populated', { missing: missing.slice() });
      if (missing.length) focusMissing(missing[0]);
    } catch (error) {
      closeSuggestions();
      publishValue(input, input.value);
      dispatch('south:address-unavailable');
      focusMissing('lead_address_address_visible');
    }
  }
  ...
  input.addEventListener('input', function () {
    if (document.body && document.body.classList) document.body.classList.remove('south-places-selection-complete');
  });
  window.SOUTH_GOOGLE_PLACES = { initializePlaces, componentValue, cityValue, handlePlaceChanged };
  const key = String(window.SOUTH_RUNTIME_CONFIG && window.SOUTH_RUNTIME_CONFIG.googleMapsBrowserKey || '').trim();
  if (!key) return;
  ...
  const callbackName = '__southGooglePlacesReady';
  window[callbackName] = initializePlaces;
  const script = document.createElement('script');
  script.async = true;
  script.defer = true;
  script.dataset.southGooglePlaces = 'true';
